...
class DepartmentModel extends BaseModel
{
     ...
     * @return array  result
     **/
    public function getBy($criteria, $value, $start = 0, $limit = 25)
    {
        $extra  = '';
     ...
     **/
    public function countTotalBy($criteria, $value)
    {
        $extra  = '';
        if ($this->getStatus() === 'true') {
            $extra  = "AND d.status = true";
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select('d.id AS department_id');
        $qb->from('GatotKacaErpMainBundle:Department', 'd');
        $qb->leftJoin('GatotKacaErpMainBundle:CompanyDepartment', 'c', 'WITH', 'd.id = c.department');
        if ($criteria == 'company') {
            $qb->where("c.{$criteria} = :{$criteria} {$extra}");
     ...
     * @return array   result
     **/
    public function getList($keyword, $start = 0, $limit = 25)
    {
        $extra  = '';
        if ($this->getStatus() === 'true') {
            $extra  = "AND d.status = true";
     ...
     * @return integer total
     **/
    public function countTotal($keyword, $limit = 25)
    {
        $extra  = '';
        if ($this->getStatus() === 'true') {
            $extra  = "AND d.status = true";
